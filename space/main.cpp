...
/*
sf::VertexArray trajectory(std::vector<std::shared_ptr<Planet> > &planets, std::shared_ptr<Spaceship> &spaceship){
    std::vector<std::shared_ptr<Planet> > cPlanets;
    for (auto const &i : planets){
        cPlanets.push_back(std::make_shared<Planet>(*i));
    }
    std::shared_ptr<Spaceship> cSpaceship = std::make_shared<Spaceship>(*spaceship);

    sf::Time dt = sf::seconds(1);
    int nPoints = 10000;
    sf::VertexArray lines(sf::LineStrip, nPoints);
    lines[0].position = cSpaceship->getPosition();
    lines[0].color = sf::Color::Yellow;
    bool crashed = false;
    for(int i=1; i<nPoints; i++){
        if(!crashed){
            gravity_forces(cPlanets, cSpaceship, dt);
            for (auto j : cPlanets)
                j->update(dt);
            cSpaceship->update(dt);
            crashed = collisions(cPlanets, cSpaceship);
            lines[i].position = cSpaceship->getPosition();
...
sf::VertexArray runge_kutta(std::vector<std::shared_ptr<Planet> > &planets, std::shared_ptr<Spaceship> &spaceship){
    auto cEarth = std::make_shared<Planet>(*planets[0]);
    auto cSpaceship = std::make_shared<Spaceship>(*spaceship);

    float G = 6.67 * pow(10,-11);

    sf::Time dt = sf::seconds(400);
    int nPoints = 20;
    ...
    for(int i=1; i<nPoints; i++){
        sf::Vector2f v1 = cEarth->getPosition() - cSpaceship->getPosition();
        sf::Vector2f u1 = v1/norme(v1);
        sf::Vector2f k1 = G*cEarth->getMass()/norme(v1)/norme(v1)*u1;

        sf::Vector2f v2 = cEarth->getPosition() - (cSpaceship->getPosition()+dt.asSeconds()/2.f*cSpaceship->getVelocity());
        sf::Vector2f u2 = v2/norme(v2);
        sf::Vector2f k2 = G*cEarth->getMass()/norme(v2)/norme(v2)*u2;

        sf::Vector2f v3 = cEarth->getPosition() - (cSpaceship->getPosition()+dt.asSeconds()/2.f*cSpaceship->getVelocity() + dt.asSeconds()*dt.asSeconds()/4.f*k1);
        sf::Vector2f u3 = v3/norme(v3);
        sf::Vector2f k3 = G*cEarth->getMass()/norme(v3)/norme(v3)*u3;

        sf::Vector2f v4 = cEarth->getPosition() - (cSpaceship->getPosition() + dt.asSeconds()*cSpaceship->getVelocity() + dt.asSeconds()*dt.asSeconds()/2.f*k2);
        sf::Vector2f u4 = v4/norme(v4);
        sf::Vector2f k4 = G*cEarth->getMass()/norme(v4)/norme(v4)*u4;


        cSpaceship->setPosition(cSpaceship->getPosition() + dt.asSeconds()*cSpaceship->getVelocity() + dt.asSeconds()*dt.asSeconds()/6.f*(k1+k2+k3));
        cSpaceship->setVelocity(cSpaceship->getVelocity() + dt.asSeconds()/6.f*(k1 + 2.f*k2 + 2.f*k3 + k4));
...
sf::VertexArray runge_kutta2(sf::Vector2f pos, sf::Vector2f vel, sf::Vector2f posEarth, float massEarth){
    float G = 6.67 * pow(10,-11);
...
int main(int, char const**)
{
    ...
    //Commentaire : uniquement la Terre
    //std::shared_ptr<Planet> earth = std::make_shared<Planet>(sf::Vector2f(0,0), sf::Vector2f(0,-12.52), 6371000.f, 5.972*pow(10, 24), sf::Color::Blue);
    //std::shared_ptr<Planet> moon = std::make_shared<Planet>(sf::Vector2f(384400000.f,0), sf::Vector2f(0,1017.96), 1737100.f, 7.342*pow(10, 22), sf::Color::Green);
    std::shared_ptr<Planet> earth = std::make_shared<Planet>(sf::Vector2f(0,0), sf::Vector2f(0,0), 6371000.f, 5.972*pow(10, 24), sf::Color::Blue);
    std::vector<std::shared_ptr<Planet> > planets;
    planets.push_back(earth);
    //planets.push_back(moon);

    std::shared_ptr<Spaceship> spaceship = std::make_shared<Spaceship>();
    //spaceship->setPosition(earth->getPosition().x, earth->getPosition().y - earth->getRadius() - 1000000);
    //spaceship->setVelocity(sf::Vector2f(sqrt(G*earth->getMass()/norme(spaceship->getPosition())),0));
    ...
    while (window.isOpen())
    {
        ...
        sf::CircleShape mark(view.getSize().x/100.f);
        ...
        //sf::VertexArray traj = trajectory(planets, spaceship);
        //sf::VertexArray traj = runge_kutta(planets, spaceship);

        sf::VertexArray traj = runge_kutta2(spaceship->getPosition(), spaceship->getVelocity(), earth->getPosition(), earth->getMass());
